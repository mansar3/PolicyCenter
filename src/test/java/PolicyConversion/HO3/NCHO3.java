...
public class NCHO3 extends BaseTest
{
	...
	@Test(dataProviderClass = AccountPolicyGenerator.class, dataProvider = "NCHO3Data",groups = "Renewal Test")
	public void RenewalLoadTest2(LinkedHashMap<String, String> eai, ArrayList<LinkedHashMap<String, String>> addInts, ArrayList<LinkedHashMap<String, String>> spp)
	{
		...
		NCHO3EnterAccountInformation enterAccountInfo = new NCHO3EnterAccountInformation(sh);
	...
	log("Test simple homeowners policy renewal");
		...
		accountFileSummary = new NCHO3AccountFileSummary(sh);
		accountFileSummary.westPanel.actions.convertManualPolicy();
		NCHO3InitiateManualRenewal imr = new NCHO3InitiateManualRenewal(sh);
...
//			imr.clickExcludedLossOfUseCoverage("false");
		NCHO3Offerings offerings = imr.nextAndAccept();

		// Offerings
		offerings
		.setPolicyType(eai.getOrDefault("Policy Type", null))
		.setOfferingSelection(eai.getOrDefault("Offering Selection","Most Popular"));
		NCHO3PolicyInfo pi = offerings.next();
		// Policy Info
		pi
		...
		if(keyContainsValue(eai,"Additional Name Insured Last Name" ) || keyContainsValue(eai,"Additional Name Insured Company Name" ))
		{
			...
			NCHO3SearchAddressBook sab = pi.searchFromAddressBook();
			...
			else
			{
				...
				if(person)
				{
					NCHO3NewAdditionalNamedInsured ani = pi.clickAddNewPerson();
					ani
				...
				else if (eai.getOrDefault("Additional Name Insured Company Name", null) != null)
				{
					...
					NCHO3NewAdditionalNamedInsured ani = pi.clickAddNewCompany();
					ani
		...
		NCHO3Dwelling dwelling = pi.next();
...
//		/Users/aansari/Downloads/ConversionPolicies-20170206_165054/NCHO3-20170206_165309
		dwelling
		...
		// Protection Details
		NCHO3Dwelling.NCHO3ProtectionDetails pd = dwelling.clickProtectionDetails();
		...
		NCHO3Dwelling.NCHO3AdditionalInterests ai = pd.clickAdditionalInterests();
				for(i= 0; i <= addInts.size() -1;i++)
		{
			...
			else
			{
				...
				NCHO3NewAdditionalInterest nai = null;
		...
		NCHO3DwellingConstruction dc = ai.next();
		...
		// Wind Mitigation
		NCHO3DwellingConstruction.NCHO3WindMitigation wm = dc.clickWindMitigation();
		wm
...
//			.setLossOfUseSelection(eai.get("Loss of Use - %"));
		...
		NCHO3Coverages.NCHO3PropertyEndorsements pe = co.clickPropertyEndorsements();
...
//		System.out.println();
		...
		{
			NCHO3RenewalBound rb = quote.renew();
	...
	public void SubmissionLoadTest(LinkedHashMap<String, String> eai, ArrayList<LinkedHashMap<String, String>> addInts, ArrayList<LinkedHashMap<String, String>> spp)
	{
		...
		NCHO3NewSubmission submission = new NCHO3NewSubmission(sh);

		submission.setBaseState(eai.get("Base State"));
		NCHO3Qualification qualification = submission.productTable.selectHomeowners();
		...
		NCHO3PolicyInfo pi = qualification.next();
		// Policy Info
		pi
		.setDoesInsuredOwnOtherResidenceWithFrontline(eai.getOrDefault("Does the insured own any other residence that is insured with Frontline?", null))
		.setSeniorCitizenDiscount(eai.get("Senior Citizen Discount"));
...
//		System.out.println();
	...
	@Test(dataProviderClass = AccountPolicyGenerator.class, dataProvider = "NCHO3Rerun")
	public void rerunRenewalTest(LinkedHashMap<String, String> eai, ArrayList<LinkedHashMap<String, String>> addInts, ArrayList<LinkedHashMap<String, String>> spp)
	{
		...
		if(accountNumber != null)
		{
			...
			{
				afs = new NCHO3AccountFileSummary(sh);
...
//
	...
	public void rerunPolicy( LinkedHashMap<String, String> eai, ArrayList<LinkedHashMap<String, String>> addInts, ArrayList<LinkedHashMap<String, String>> spp, boolean accountNumberPresent, boolean transactionPresent)
	{
		...
		if(!accountNumberPresent)
		{
			...
			//.setProducerCode("012-13-12345 ");
			NCHO3AccountFileSummary accountFileSummary = createAccount.checkForDuplicatesAndReturn().clickUpdate();
		...
		NCHO3Offerings offerings;
		if(!transactionPresent)
		{
			new NCHO3AccountFileSummary(sh).westPanel.actions.convertManualPolicy();
		...
		else
		{
			...
			offerings = new NCHO3Offerings(sh, CenterPanelBase.Path.POLICYRENEWAL);
