...
{
	@Test(dataProviderClass = AccountPolicyGenerator.class, dataProvider = "SCHO3Data",groups = "Renewal Test")
...
//		System.out.println();
		...
		{
			SCHO3RenewalBound rb = quote.renew();
...
//		System.out.println();
	...
	@Test(dataProviderClass = AccountPolicyGenerator.class, dataProvider = "SCHO3Rerun")
	public void rerunRenewalTest(LinkedHashMap<String, String> eai, ArrayList<LinkedHashMap<String, String>> addInts, ArrayList<LinkedHashMap<String, String>> spp)
	{
		...
		if(accountNumber != null)
		{
			...
			{
				afs = new SCHO3AccountFileSummary(sh);
	...
	public void rerunPolicy( LinkedHashMap<String, String> eai, ArrayList<LinkedHashMap<String, String>> addInts, ArrayList<LinkedHashMap<String, String>> spp, boolean accountNumberPresent, boolean transactionPresent)
	{
		...
		if(!accountNumberPresent)
		{
			...
			SCHO3AccountFileSummary accountFileSummary = createAccount.checkForDuplicatesAndReturn().clickUpdate();
		...
		SCHO3Offerings offerings;
		if(!transactionPresent)
		{


			new SCHO3AccountFileSummary(sh).westPanel.actions.convertManualPolicy();
			SCHO3InitiateManualRenewal imr = new SCHO3InitiateManualRenewal(sh);
			// Initiate Manual Renewal
			imr//.setOrganization(eai.getOrDefault("Organization", null))
			//.setProducerCode(eai.getOrDefault("Producer Code", null))
			.setBaseState(eai.getOrDefault("Base State", null)).setProduct(eai.getOrDefault("Product", null)).setPolicyType(eai.getOrDefault("Policy Type", null)).setLegacyPolicyNumber(eai.getOrDefault("Legacy Policy Number", null)).setOriginalEffectiveDate((eai.getOrDefault("Policy Original Effective Date", null))).setEffectiveDate(eai.getOrDefault("Effective Date", null)).setLastInspectionCompletionDate(eai.get("Last Inspection Completion Date"));
			if(!eai.getOrDefault("Inflation Guard", "none").toLowerCase().equals("none"))
				imr.setInflationGuard(eai.getOrDefault("Inflation Guard", null));
			if(!eai.get("Exclude Loss of Use Coverage").toLowerCase().equals("false"))
				imr.clickExcludedLossOfUseCoverage("true");
		...
		else
		{
			...
			offerings = new SCHO3Offerings(sh, CenterPanelBase.Path.POLICYRENEWAL);
